...
- 初始实现按用户要求跳过真机测试，只提交不推送；后续由 Grok (Cursor Agent) 在 OnePlus 7 Pro 上完成真机补测，详见 `tests/v0.01.0022_2026-06-20_145553/REPORT.md`。
- 真机验证通过：`64` 显示 `ni`/`mi` 拼音选择条，点击 `mi` 后候选刷新为"米/密/迷/谜/秘/觅"，追加数字后选择状态清空；补测 v0.01.0021 遗留项，空 buffer 下 `0` 键插入空格通过。
- 同次测试完成词库冷启动加载性能首测：OnePlus 7 Pro 冷进程 5 轮为 **622–977 ms**（均值约 892 ms），高于 200–300 ms 的无感参考线；临时 `PinyinDictPerf` 日志仅用于测试，提交前已移除。
